...
const scpTo = (local, remote, timeout = 15000) => {
  return new Promise((resolve, reject) => {
    const child = spawn('scp', [
      ...
      '-P', String(ROUTER_PORT),
      local,
      ROUTER_USER + '@' + ROUTER_HOST + ':' + remote
    ], { timeout, stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true });
    let stderr = '';
    child.stderr.on('data', d => { stderr += d.toString(); });
    child.on('close', code => {
...
app.post('/api/router/pull', async (req, res) => {
  ...
  try {
    ...
    if (type === 'subconverter' || type === 'all') {
      try { await fs.mkdir(path.join(configDir, 'subconverter_rules'), { recursive: true }); } catch {}
      await runSSH('tar czf /tmp/sub_rules.tgz -C /etc/subconverter rules/ groups.txt pref.ini emoji.txt', 30000);
      await scpFrom('/tmp/sub_rules.tgz', path.join(configDir, 'subconverter.tgz'), 30000);
      try {
        const { spawn } = await import('child_process');
        await new Promise((resv, rej) => {
          const c = spawn('tar', ['xzf', path.join(configDir, 'subconverter.tgz'), '-C', path.join(configDir, 'subconverter_rules')], { stdio: 'ignore', windowsHide: true });
          c.on('close', code => code === 0 ? resv() : rej(new Error('tar exit ' + code)));
          c.on('error', rej);
        });
        try { await fs.unlink(path.join(configDir, 'subconverter.tgz')); } catch {}
      } catch(e) { /* tar extraction failed, subconverter files may be incomplete */ }
      for (const sf of ['groups.txt', 'pref.ini', 'emoji.txt']) {
        await scpFrom('/etc/subconverter/' + sf, path.join(configDir, 'subconverter_' + sf), 15000);
      }
      r.push('subconverter');
    }
    if (type === 'openclash' || type === 'all') {
      ...
      for (const f of configFiles.stdout.trim().split('\n').filter(Boolean)) {
        await scpFrom(f, path.join(configDir, 'openclash_' + f.split('/').pop()), 15000);
      }
      // 也拉取根目录的快捷副本
      const rootYamls = await runSSH('ls /etc/openclash/*.yaml 2>/dev/null || true', 10000);
      for (const f of rootYamls.stdout.trim().split('\n').filter(Boolean)) {
        await scpFrom(f, path.join(configDir, 'openclash_root_' + f.split('/').pop()), 15000);
      }
      r.push('openclash');
    }
...
app.post('/api/router/delete-file', async (req, res) => {
  ...
  if (!rpath) return res.status(400).json({ success: false, error: 'Missing path' });
  try {
    await runSSH('rm -f ' + rpath.replace(/'/g, "'\\''"), 10000);
